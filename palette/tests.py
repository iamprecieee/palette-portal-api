...
class GenreDetailTestCase(APITestCase):
    ...
    def test_put_genre_slug_not_name(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual("Update error. You can only update the Name field. Slug field updates automatically.", response.data)
...
class ArtworkDetailTestCase(APITestCase):
    ...
    def test_put_artwork(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual("Update error. You cannot edit the values of name, artist.", response.data)
    
        
        response2 = self.client.put(self.url, data={
            "is_available": False
        })
...
class CartDetailTestCase(APITestCase):
    def setUp(self):
        ...
        self.url = reverse("palette:cart-detail", kwargs={"id": self.artwork.id})
            
    def test_post_cart_detail(self):
        response = self.client.post(reverse("palette:cart-detail", kwargs={"id": "4d9dc378-24aa-4148-a8f7-772aad7915d7"}))
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_put_cart_detail(self):
        response = self.client.put(reverse("palette:cart-detail", kwargs={"id": "4d9dc378-24aa-4148-a8f7-772aad7915d7"}))
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        self.assertEqual("Artwork not found.", response.data)
